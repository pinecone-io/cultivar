...
Want to use skill-eval with an agent? Point it at this repo, or ask it to call --help to learn the tool!
...
```bash
uv tool install skill-eval

# Or install from source:
uv tool install --from "git+https://github.com/pinecone-io/skill-eval" skill-eval
```
...
|---|---|---|
| `SKILL_EVAL_MODAL_SECRET` | `eval-sandbox-secrets` | Name of the Modal secret mounted into each sandbox |
| `SKILL_EVAL_MODAL_APP` | `skill-evals` | Modal app name (useful for isolating runs across teams or projects) |
...
```bash
skill-eval run --skill my-skill --runner claude --remote --grade
```
...
```bash
skill-eval hello                      # local: agent + grader (needs ANTHROPIC_API_KEY)
skill-eval hello --remote             # also exercises Modal + eval-sandbox-secrets
skill-eval hello --no-grade           # just exercise the runner (no API key needed)
```

`hello` runs a tiny "write hello.py" task that ships inside the wheel — no repo clone, no `tasks/` setup. It exits 0 on PASS and prints diagnostics on FAIL. Use this to learn how to use skill-eval.
...
skill-eval run --skill my-skill --runner claude --task my-task -v with-skill --remote

# All tasks + every applicable variant (with-skill, without-skill, and with-docs
# for tasks that declare context_refs)
skill-eval run --skill my-skill --runner claude --remote

# 3 runs per (task, variant) for reliability, 5 sandboxes at once
skill-eval run --skill my-skill --runner claude --remote --repeat 3 --parallel 5

# Raise the per-call wall-clock budget (default 90s; sandbox gets +60s buffer)
skill-eval run --skill my-skill --runner claude --remote --timeout 180

# All three runners in parallel
skill-eval run --skill my-skill --runner claude --remote &
skill-eval run --skill my-skill --runner copilot --remote &
skill-eval run --skill my-skill --runner gemini --remote &

# Run + grade in one shot
skill-eval run --skill my-skill --runner claude --remote --grade

# Name a run so you can tell it apart later
skill-eval run --skill my-skill --runner claude --remote --title baseline
skill-eval run --skill my-skill --runner claude --remote --title after-tweak
```
...
├── grades.json                                # written by grader after `skill-eval grade`
...
| What | Where to look |
|---|---|
| One run, all sections (conversation, stats, workdir, grader) | `skill-eval show latest -r claude -t <task>` |
| Just the conversation transcript for one run | `skill-eval show latest -t <task> --conversation-only` |
| Just the grader verdict + reasoning + suggestions | `skill-eval show latest -t <task> --grader` |
| Just the workdir file listing | `skill-eval show latest -t <task> --workdir` |
| Summary table across all runners + variants | `skill-eval report` |
...
| Grader verdict + evidence + reasoning + suggestions | `grades.json` or `skill-eval report latest` |
...
Want to use skill-eval with a team, but don't want to make everyone have different Modal workspaces?
...
2. **On their machine:**
   ...
   skill-eval init my-skill            # scaffolds tasks/my-skill.yaml
   skill-eval run --skill my-skill --runner claude --remote --grade
   ```
...
- [docs/concepts.md](docs/concepts.md) — **start here** if you're new: what skill-eval measures, why, and how to read the results
...
For any subcommand: `skill-eval <cmd> --help`.
